...
class BendLikePanel(SimplePanel):
    ...
    def gen_side_types(self):
        self.niche = Side(self.edges[0])
        self.bottom = Bottom(self.edges[2])
        self.side = [BoardHolesOne(self.edges[1], False), BoardHolesTwo(self.edges[3], True)]
...
class BoardPanel(MainPanel):
    ...
    def cut(self):
        ...
        side = rh.Curve.JoinCurves([side, self.extra_panel.cut[0]])[0]
        return [side]

    @property
    def fres(self):
        fres = [self.side[1].fres.DuplicateCurve(), self.niche.fres.DuplicateCurve(),
                self.side[0].fres.DuplicateCurve()]
        [i.Transform(self.bound_plane) for i in fres]

        join = []
        for i, v in zip(fres, self.extra_panel.fres):
            s = rh.Curve.JoinCurves([i,v])
            join.extend(s)

        bot = self.bottom.fres.DuplicateCurve()
        bot.Transform(self.bound_plane)
        join.append(bot)
        return join
    ...
    def __init__(self, surf, tag=None, cogs_bend=None, holes=None, **kwargs):
        ...
        self.extra_panel = self.create_panel(self.bottom.fres)
        setattr(self.extra_panel, 'bound_plane', self.bound_plane)
