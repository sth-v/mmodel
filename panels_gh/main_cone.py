...
class Panel:
    ...
    def __init__(self, surface, type):
        ...
        #self.niche_otgib =[self.niche.edge,self.bottom.edge, self.side[0].edge, self.side[1].edge]
    ...
    def gen_side_types(self):
        ...
        if self.type == 0:
                ...
                self.side = [Side(self.edges[3], self.surface, self.type, 'left'),
                             Side(self.edges[1], self.surface, self.type, 'right')]
        else:
                self.niche = Niche(self.edges[0], self.surface, self.type)
                self.bottom = Bottom(self.edges[2], self.surface, self.type)
